...
export default function EditDeckPage() {
  ...
  const handleEditDeck = async (data: DeckCreatePayload) => {
    ...
    try {
      // Use the deck's ID for the PATCH request, but we got here via slug
      await api.patch(`/decks/${deckData?.id}`, data)
  ...
  if (deckError || !deckData)
    return <p className="text-destructive text-center py-10">Error loading deck data or deck not found.</p>
  if ((deckData.author as any)?.id !== user.id)
    return <p className="text-destructive text-center py-10">You are not authorized to edit this deck.</p>

  return (
    <div className="py-8">
      <DeckForm initialData={deckData} onSubmitForm={handleEditDeck} isSubmitting={isSubmitting} />
    </div>
  )
}
